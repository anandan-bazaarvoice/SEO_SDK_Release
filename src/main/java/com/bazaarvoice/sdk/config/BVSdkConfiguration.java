...
public class BVSdkConfiguration implements BVConfiguration {
	...
	static {
		...
		if (inputStream == null) {
			throw new BVSdkException("ERR0003");
		}
		...
		try {
			...
			_logger.error(BVSdkMessageUtil.getMessage("ERR0004"), e);
			throw new BVSdkException("ERR0004", e);
		}
		
		_logger.info(BVSdkMessageUtil.getMessage("MSG0000"));
	...
	private static void loadBvClient() {
		...
		if (!bvClientFile.exists() || !bvClientFile.isFile()) {
			_logger.info(BVSdkMessageUtil.getMessage("MSG0002"));
			return;
		}
		
		_logger.info(BVSdkMessageUtil.getMessage("MSG0001"));
        try {
        	...
        	_logger.error(BVSdkMessageUtil.getMessage("ERR0005"), e);
